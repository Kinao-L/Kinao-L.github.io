# Leaflet cluster map of travel locations
#
# Run this script from the root directory of the GitHub Pages project.
# It reads location information from Markdown files in _travels/,
...
markdown_files = glob.glob("_travels/*.md")

# Prepare geocoder
geocoder = Nominatim(user_agent="Kinao-L.github.io-travelmap")
...
for file_path in markdown_files:
    ...
    title = str(data.get("title", "Untitled Travel")).strip()
    ...
    for location in locations:
        # Create the text shown when users click the map marker
        ...
        # Optionally make the title clickable
        if permalink:
            description = (
        ...
        try:
            ...
            # Prevent duplicate dictionary keys when descriptions are identical
            unique_description = description
            duplicate_number = 2

            while unique_description in location_dict:
                unique_description = (
                    f"{description}<br />Location {duplicate_number}"
                )
                duplicate_number += 1
...
if not location_dict:
    ...
    getorg.orgmap.output_html_cluster_map(
        location_dict,
        folder_name="travelmap",
        hashed_usernames=False
    )

    print(
        f"Travel map generated successfully with "
